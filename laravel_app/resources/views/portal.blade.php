...
<head>
  ...
  <title>AUS — Author Resource Portal</title>
  <script src="https://cdn.tailwindcss.com"></script>
  <script src="https://unpkg.com/lucide@latest"></script>
  <script>tailwind.config = { theme: { extend: { colors: { navy: '#093163', aus: '#1976df', ink: '#0b3470', mist: '#f5faff' }, boxShadow: { soft: '0 7px 18px rgba(25,91,171,.08)' } } } }</script>
  ...
  <style>
    ...
    .sidebar {
      background: linear-gradient(160deg, #103e75, #062653)
    }
    ...
    .nav-link.active,
    .nav-link:hover {
      background: linear-gradient(90deg, #1d7ce5, #1769ca);
      box-shadow: 0 8px 15px #051f4629
    }
    ...
    .app-hero {
      ...
      background: linear-gradient(135deg, #ffffff 0%, #eff7ff 58%, #d7ebff 100%);
...
<body class="min-h-screen">
  <aside id="sidebar" class="sidebar fixed inset-y-0 left-0 z-40 hidden w-[16.75rem] flex-col text-white lg:flex">
    <div class="px-5 pb-5 pt-5">
      <div class="text-[42px] font-extrabold leading-none tracking-tight">AUS</div>
      <p class="mt-1 text-sm text-blue-100">Author Resource Portal</p>
    </div>
    <nav id="sideNav" class="space-y-1 px-3"></nav>
    <div class="mt-auto border-t border-white/10 p-4 text-xs text-blue-200">Tellwell PC Cheat Sheet V1.0</div>
  </aside>
  <div class="lg:pl-[16.75rem]">
    <header
      ...
      <button id="menuBtn" class="rounded-lg p-2 text-ink hover:bg-blue-50 lg:hidden" aria-label="Open navigation"><i
          ...
          class="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#1a5caa]"></i><input id="globalSearch"
          placeholder="Search resources, guides, and more..."
          class="w-full rounded-lg bg-[#edf4fc] py-2.5 pl-11 pr-4 text-sm text-ink outline-none ring-aus focus:ring-2"></label>
      <div class="ml-auto flex items-center gap-3"><button id="addBtn"
        ...
        <div class="grid h-10 w-10 place-items-center rounded-full bg-[#176bd0] text-xs font-bold text-white">AE</div>
      </div>
    </header>
    <main class="mx-auto max-w-[1500px] px-4 pb-10 pt-6 sm:px-6">
      <div id="pageContent">
        <section
          class="relative mb-5 overflow-hidden rounded-2xl bg-gradient-to-r from-white via-[#f4f9ff] to-[#dceeff] px-5 py-7 sm:px-8">
          <div class="relative z-10 flex items-center gap-4">
            <div class="icon-ball grid h-14 w-14 place-items-center rounded-full bg-[#d9edff] text-[#1169cc]"><i
                data-lucide="book-open" class="h-7 w-7"></i></div>
            <div>
              <h1 class="text-2xl font-extrabold tracking-tight text-[#0b3470] sm:text-[32px]">Consultation Resources
              </h1>
              <p class="mt-1 text-sm text-[#5274a2] sm:text-base">Everything you need, organized in one place.</p>
            </div>
          </div>
          <div class="absolute right-6 top-4 hidden opacity-70 md:block"><i data-lucide="library-big"
              class="h-32 w-32 text-[#5da5eb]"></i></div>
        </section>
        <section class="card mb-4 rounded-xl bg-white p-3">
          <div class="flex flex-col gap-3 xl:flex-row xl:items-center"><label
              class="relative w-full xl:max-w-[560px]"><i data-lucide="search"
                class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#1260b8]"></i><input id="resourceSearch"
                placeholder="Search consultation resources..."
                class="w-full rounded-lg border border-[#d4e6fb] py-2.5 pl-10 pr-3 text-sm outline-none ring-aus focus:ring-2"></label>
            <div id="filters" class="flex flex-wrap gap-2"></div><button id="favoritesOnly"
              class="ml-auto inline-flex items-center gap-1 rounded-full border border-blue-100 px-3 py-2 text-xs font-semibold text-[#386198] hover:bg-blue-50"><i
                data-lucide="star" class="h-3.5 w-3.5"></i> Saved</button>
          </div>
        </section>
        <div id="resultInfo" class="mb-3 hidden text-xs font-medium text-[#5274a2]"></div>
          ...
          <div class="card rounded-xl bg-white p-4">
            <div class="mb-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-[11px] text-[#5274a2]">Most accessed by authors and team members.</p>
              </div>
            </div>
          ...
          <div class="card rounded-xl bg-white p-4">
            <div class="mb-3 flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-[11px] text-[#5274a2]">Your latest resource activity.</p>
              </div>
            </div>
            <div id="recent"></div>
          </div>
  ...
  <script>
    ...
    const tabPages = {
      1: {
        eyebrow: 'Reference guide', title: 'Country Code Guide', intro: 'A quick country-calling-code reference for the LATAM and EUR regions. Use it before calling or validating an international phone number.', groups: [
          { title: 'LATAM region', icon: 'map-pinned', items: ['Argentina — +54, +549', 'Bolivia — +591', 'Brazil — +55', 'Chile — +56', 'Colombia — +57', 'Costa Rica — +506', 'Cuba — +53', 'Dominican Republic — +1 809 / +1 829 / +1 849', 'Ecuador — +593', 'Mexico — +52', 'Peru — +51', 'Puerto Rico — +1 787 / +1 939', 'Uruguay — +598'] },
          { title: 'EUR region', icon: 'map-pinned', items: ['Albania — +355', 'Andorra — +376', 'Austria — +43', 'Belgium — +32', 'France — +33', 'Germany — +49', 'Ireland — +353', 'Italy — +39', 'Netherlands — +31', 'Portugal — +351', 'Spain — +34', 'Switzerland — +41', 'United Kingdom — +44'] }
        ]
      },
      2: {
        eyebrow: 'Call toolkit', title: 'Spiels', intro: 'Ready-to-use phone guidance, callback details, and conversation templates for publishing consultations.', groups: [
          { title: 'Toll-free callback numbers', icon: 'phone-call', items: ['Canada — 1-888-415-1541', 'United States — 1 (800) 891-4160', 'Australia — 61 (1800) 934-224', 'Leave the toll-free number with the caller’s extension as the callback number.'] },
      ...
      3: {
        eyebrow: 'Billing reference', title: 'Billing & Invoicing', intro: 'Discounting limits, payment-plan rules, and the internal process for company forms.', groups: [
          { title: 'Regular discounting guidelines', icon: 'badge-dollar-sign', items: ['Invoice totals: CA$2,199 · AU$2,399 · US$1,749 · £1,249 · €1,499', 'Maximum cash discount: 10%', 'Maximum free-service value: CA$330 · AU$360 · US$262 · £187 · €225'] },
          { title: 'Payment plans & company forms', icon: 'file-check-2', items: ['Full payment is available for qualifying invoices.', 'A 2–3 month payment plan includes an admin fee.', 'Payments must be 30 days apart.', 'Complete the required details, then forward the form to JH for signing.'] }
        ]
      },
      4: {
        eyebrow: 'Internal directory', title: 'Directory & Company Info', intro: 'Core Tellwell company details and a central place to find the appropriate team member.', groups: [
          { title: 'Tellwell at a glance', icon: 'building-2', items: ['Tellwell Talent, Inc. — founded 2015', 'Headquarters: Victoria, British Columbia, Canada', 'Satellite office: Tellitwell International Inc., Cebu, Philippines — founded 2018', 'Workforce: 60 full-time staff and around 50 contractors', 'Official address: 2031 Store Street, Victoria, BC, V8T 5L9'] },
      ...
      5: {
        eyebrow: 'Daily operations', title: 'Cadences / Workflows', intro: 'A practical guide for planning the day, prioritizing leads, and maintaining consistent follow-up.', groups: [
          ...
          { title: 'Daily productivity guide', icon: 'calendar-check-2', items: ['Read and respond to unreplied email and Slack/Teams messages from the prior day.', 'Identify the day’s top 20 warmest leads.', 'Organize the workday around the priority queue and planned follow-ups.', 'Start the day prepared, focused, and ready for client conversations.'] }
        ]
      },
      6: {
        eyebrow: 'Learning centre', title: 'Learning & Resources', intro: 'Team videos and quick-reference links from the Tellwell training library.', groups: [
          { title: 'Tutorial videos', icon: 'video', items: ["The Evolution of Self-Publishing with Tellwell's Founder & CEO Tim Lindsay", '8 Tips for Choosing the Right Self-Publishing Company for Your Book', 'Debunking 15 Self-Publishing Myths with Tellwell Publishing Consultant Mitch Anderson', 'Is Self-Publishing Legitimate? The Case for Self-Publishing', 'Navigating Vulnerability in Memoir Writing with Self-Published Author Karen Harmon', 'Increase Your Amazon Ranking (And Book Sales!) With These Tips!', 'The Evolution of Book Promotion with Author Karen Cumming', '7 Marketing Tips to Get Your Children’s Book Noticed!'] },
    ...
    // Country Code Guide data, transcribed from the workbook's DialCodes and LocalPatterns tabs.
    const dialCodes = [
      ['54', 'Argentina', 'LATAM'], ['591', 'Bolivia', 'LATAM'], ['55', 'Brazil', 'LATAM'], ['56', 'Chile', 'LATAM'], ['57', 'Colombia', 'LATAM'], ['506', 'Costa Rica', 'LATAM'], ['53', 'Cuba', 'LATAM'], ['1809', 'Dominican Republic', 'LATAM'], ['1829', 'Dominican Republic', 'LATAM'], ['1849', 'Dominican Republic', 'LATAM'], ['593', 'Ecuador', 'LATAM'], ['503', 'El Salvador', 'LATAM'], ['502', 'Guatemala', 'LATAM'], ['509', 'Haiti', 'LATAM'], ['504', 'Honduras', 'LATAM'], ['1876', 'Jamaica', 'LATAM'], ['52', 'Mexico', 'LATAM'], ['505', 'Nicaragua', 'LATAM'], ['507', 'Panama', 'LATAM'], ['595', 'Paraguay', 'LATAM'], ['51', 'Peru', 'LATAM'], ['1787', 'Puerto Rico', 'LATAM'], ['1939', 'Puerto Rico', 'LATAM'], ['598', 'Uruguay', 'LATAM'], ['58', 'Venezuela', 'LATAM'],
      ['355', 'Albania', 'EUR'], ['376', 'Andorra', 'EUR'], ['43', 'Austria', 'EUR'], ['375', 'Belarus', 'EUR'], ['32', 'Belgium', 'EUR'], ['387', 'Bosnia & Herzegovina', 'EUR'], ['359', 'Bulgaria', 'EUR'], ['385', 'Croatia', 'EUR'], ['357', 'Cyprus', 'EUR'], ['420', 'Czechia', 'EUR'], ['45', 'Denmark', 'EUR'], ['372', 'Estonia', 'EUR'], ['358', 'Finland', 'EUR'], ['33', 'France', 'EUR'], ['49', 'Germany', 'EUR'], ['350', 'Gibraltar', 'EUR'], ['30', 'Greece', 'EUR'], ['36', 'Hungary', 'EUR'], ['354', 'Iceland', 'EUR'], ['353', 'Ireland', 'EUR'], ['39', 'Italy', 'EUR'], ['383', 'Kosovo', 'EUR'], ['371', 'Latvia', 'EUR'], ['423', 'Liechtenstein', 'EUR'], ['370', 'Lithuania', 'EUR'], ['352', 'Luxembourg', 'EUR'], ['356', 'Malta', 'EUR'], ['373', 'Moldova', 'EUR'], ['377', 'Monaco', 'EUR'], ['382', 'Montenegro', 'EUR'], ['31', 'Netherlands', 'EUR'], ['389', 'North Macedonia', 'EUR'], ['47', 'Norway', 'EUR'], ['48', 'Poland', 'EUR'], ['351', 'Portugal', 'EUR'], ['40', 'Romania', 'EUR'], ['378', 'San Marino', 'EUR'], ['381', 'Serbia', 'EUR'], ['421', 'Slovakia', 'EUR'], ['386', 'Slovenia', 'EUR'], ['34', 'Spain', 'EUR'], ['46', 'Sweden', 'EUR'], ['41', 'Switzerland', 'EUR'], ['90', 'Turkey', 'EUR'], ['380', 'Ukraine', 'EUR'], ['44', 'United Kingdom', 'EUR'], ['379', 'Vatican City', 'EUR']
    ];
    const localPatterns = [
      ['EUR', '^07\\d{9}$', 'United Kingdom', 'Medium', 'Typical UK mobile local format'], ['EUR', '^08\\d{8}$', 'Ireland', 'Medium', 'Common Irish mobile local format'], ['EUR', '^[67]\\d{8}$', 'Spain', 'Medium', 'Common Spanish mobile local format'], ['EUR', '^0[1-9]\\d{8}$', 'France', 'Low', 'Broad French national format'], ['EUR', '^9\\d{8}$', 'Portugal', 'Medium', 'Common Portuguese mobile format'], ['EUR', '^07\\d{8}$', 'Romania', 'Medium', 'Common Romanian mobile format'], ['LATAM', '^09\\d{8}$', 'Ecuador', 'Medium', 'Common Ecuador mobile local format'], ['LATAM', '^9\\d{8}$', 'Peru', 'Medium', 'Common Peru mobile local format'], ['LATAM', '^3\\d{9}$', 'Colombia', 'Medium', 'Common Colombia mobile format'], ['LATAM', '^04(12|14|16|24|26)\\d{7}$', 'Venezuela', 'Medium', 'Common Venezuela mobile prefixes'], ['LATAM', '^8\\d{7}$', 'Dominican Republic', 'Low', 'Broad DR local format']
    ];
    const companyInfo = [
      ['Name', 'Tellwell Talent, Inc. (Founded 2015)'], ['Headquarters', 'Victoria, BC (Canada)'], ['Satellite Offices', 'Tellwell International Inc., Cebu, Philippines (Founded 2018)'], ['Workforce', '60 full time and around 50 contractors'], ['Official Address', '2031 Store Street, Victoria, BC, V8T 5L9'], ['Market Presence', 'Canada, USA, Australia, UK and major EU countries'], ['Corporate Website', 'tellwellpublishing.com'], ['Toll-Free Numbers', '1-888-415-1541 (Canada) · 1 (800) 891-4160 (USA) · 61 (1800) 934-224 (Australia) · 44 0800 058 4645 (UKI)'], ['Email Contact', 'contact@tellwellpublishing.com'], ['Published Books', 'More than 4,000 titles from more than 1,500 authors across the globe'], ['Founder & CEO', 'Timothy Lindsay']
    ];
    const teamDirectory = [
    ...
    const portalDetail = {
      2: { label: 'Spiels', intro: 'Ready-to-use conversation guidance for publishing consultations.', groups: [['Opening / greeting spiels', ['Standard Inbound Greeting|Hi, thanks for calling Tellwell — this is [Name], one of our Publishing Consultants. I understand you are working on a book — congratulations! Do you have a few minutes so I can learn more about your project and how we can help?', 'Warm Callback (Web Form Lead)|Hi [Author Name], this is [Name] from Tellwell — you submitted a request about publishing your book, so I wanted to follow up personally. Is now still a good time to chat for about fifteen minutes?']], ['Objection handling spiels', ['Price Objection — Package Comparison|I hear you — investment is a big part of this decision. Can I walk you through what is included in each package so we can find the best fit for your budget and goals?', 'Let Me Think About It|Of course, this is an important decision and I want you to feel confident. What is the biggest question still on your mind — the investment, timeline, or process?', 'I Found It Cheaper Elsewhere|That is fair to compare. What is included in that quote — specifically editing rounds, distribution, and who owns the rights? I want to make sure it is an apples-to-apples comparison.']], ['Follow-up & closing spiels', ['3-Day Silent Follow-Up|Hi [Author Name], just following up after our conversation earlier this week. I know publishing a book is a big decision, so no pressure — I wanted to check whether any new questions came up.', 'Re-engagement After 30 Days|Hi [Author Name], it has been a little while since we connected about your book. Is this still something you are planning to move forward with this year?', 'Assumptive Close|Based on everything we discussed, the [Package Name] sounds like the right fit. Should we go ahead and get your project started today?', 'Timeline-Based Close|Since you mentioned wanting the book out before [target date], we would need to lock in the schedule this week. Would you like me to send the agreement?']], ['Refund / cancellation spiels', ['Cancellation Request — Early Stage|I am sorry to hear you are looking to step away from your project. Before we process anything, can you share what changed? I want to make sure we have done everything we can to support you first.', 'Refund Policy Explanation|I understand, and I want to be transparent. Per policy, deposits become non-refundable once production work has begun, but milestones not yet started may be eligible. Let me pull up your project file and walk through where things stand.']]] },
      3: { label: 'Billing & Invoicing', intro: 'Reference rules for payments, corrections, refunds, and account follow-up.', groups: [['Payment terms & methods', ['Accepted methods: credit card, e-transfer for CAD accounts, and wire transfer for international authors.', 'Installment plans are available on packages over $2,500 and require Team Lead approval.', 'Invoices are due within 7 days unless the agreement states otherwise.']], ['Payment, refund & overdue procedures', ['A 50% deposit is required before production begins on a publishing package.', 'Deposits are non-refundable once editorial or design work has begun.', 'Unused milestone payments may be refundable, minus a 5% processing fee.', 'Invoices unpaid after 7 days trigger reminders; work pauses when balances are more than 14 days overdue.', 'Accounts 30+ days overdue are escalated to Accounts for collections review.']], ['Invoice management rules', ['Do not edit a sent invoice directly — void it and issue a corrected version referencing the original.', 'Voided invoices need a one-line reason in internal notes.', 'Only Team Leads and Accounts staff may void invoices over $1,000.', 'Upgrades are billed as the package-price difference, prorated for work already completed.', 'Downgrades are permitted only before production begins and need Team Lead sign-off.']], ['Currency rules', ['CAN-market authors are invoiced in CAD; USA, AUS, and UKI authors are invoiced in local currency where supported.', 'The exchange rate is locked on the invoice issue date.', 'International wire transfers must cover sending-side bank fees.']]] },
      5: { label: 'Cadences / Workflows', intro: 'Step-by-step operational timelines from author onboarding through post-publication follow-up.', groups: [['New Author Onboarding Cadence', ['Day 1 — Welcome Call|Confirm the package, expectations, and next steps after the agreement is signed.', 'Day 1 — Welcome Email & Portal Access|Send author-portal credentials and an onboarding PDF.', 'Day 2 — Project Handoff to PM|Brief the assigned Project Manager on goals, timeline, and special notes.', 'Day 3–5 — Kickoff Call with PM|Confirm manuscript submission date and outline the production roadmap.', 'Week 2 — Manuscript Intake Reminder|Check in if the manuscript has not yet been received.', 'Week 3 — First Check-In Survey|Confirm the author feels supported and identify open questions.']], ['Manuscript-to-Production Workflow', ['Upon submission — Manuscript Received|Log the manuscript, check formatting, and confirm word count.', 'Within 2 business days — Assigned to Editorial|Route the manuscript to an editor based on genre and package tier.', '2–4 weeks — First Editing Pass|Complete the appropriate editing pass and return tracked changes.', '2 weeks — Author Review Window|Author reviews edits and returns approval or requested changes.', '2–3 weeks — Design & Typesetting|Produce interior layout and cover design.', '1 week — Proof Review|Author reviews the digital proof; the PM logs final corrections.', 'Upon approval — Final File Release|Release print-ready files to distribution partners.']], ['Post-Publication Follow-Up', ['Day of publication — Launch Congratulations Call|Celebrate the launch and confirm author copies are on order.', 'Week 1 — Marketing Services Check-In|Discuss press-release distribution and promotional add-ons.', 'Day 30 — Sales Snapshot|Share early sales/ranking data and discuss next marketing steps.', 'Day 45 — Review Request Nudge|Prompt the author to ask early readers for reviews.', 'Month 6 — Revisions Check-In|Discuss post-publication revisions and the second-edition process.']]] },
      6: { label: 'Learning & Resources', intro: 'Training references and videos for publishing consultants.', groups: [['Tutorial videos', ["The Evolution of Self-Publishing with Tellwell's Founder & CEO Tim Lindsay", "8 Tips for Choosing the Right Self-Publishing Company for Your Book", 'Debunking 15 Self-Publishing Myths with Tellwell Publishing Consultant Mitch Anderson', 'Is Self-Publishing Legitimate? The Case for Self-Publishing', 'Navigating Vulnerability in Memoir Writing with Self-Published Author Karen Harmon', 'Increase Your Amazon Ranking (And Book Sales!) With These Tips!', 'The Evolution of Book Promotion with Author Karen Cumming', '7 Marketing Tips to Get Your Children’s Book Noticed!']], ['Quick reference links', ['Tellwell Size Matrix', 'Print Prices', 'Publishing Timeline / Turn-around-time', 'Production Process Overview', 'Manuscript Submission Guidelines']]] }
    };
    function detectCountry(phone, region = '') { let raw = String(phone || '').trim(), digits = raw.replace(/\D/g, ''), selected = String(region).trim().toUpperCase(); if (!raw) return { status: '', detail: 'Enter a phone number to check it.' }; let international = raw.startsWith('+') || digits.startsWith('00'); if (international) { let normalized = raw.startsWith('+') ? digits : digits.replace(/^00/, ''); let candidates = dialCodes.filter(([code, , area]) => (!selected || area === selected) && normalized.startsWith(code)); if (!candidates.length) return { status: 'Needs manual confirmation', detail: 'No matching country code was found in the selected region.' }; let longest = Math.max(...candidates.map(([code]) => code.length)), matches = candidates.filter(([code]) => code.length === longest); return { status: 'Exact / high confidence', detail: matches.map(([code, country]) => `${country} (+${code})`).join(' · ') }; } let matches = localPatterns.filter(([area, pattern]) => (!selected || area === selected) && new RegExp(pattern).test(digits)); if (matches.length) return { status: 'Possible match', detail: matches.map(([, , country, confidence, note]) => `${country} — ${confidence} confidence (${note})`).join(' · ') }; return { status: 'Needs manual confirmation', detail: 'Use a number beginning with + or 00 for an exact match, or select a region for local-format matching.' }; }
    ...
    const leadTaggingGuide = [
      ['Attempted 1–7', 'You made your first to seventh attempt to contact the lead by phone, email, and text, with no response.'],
    ...
    const reassignmentGuides = [
      { title: 'Leads created before April 1, 2022', rows: [['Uncontacted', '> 3 business days'], ['Attempted 1–7', 'Last activity/open task due date is > 30 days old'], ['Communicating', 'Last activity/open task due date is > 1 year old'], ['Regardless of status', 'Lead owner is no longer with Tellwell']] },
      { title: 'Leads created April 1, 2022 and after', rows: [['Uncontacted', '> 2 business days'], ['Attempted 1–7', '> 30 days old'], ['Communicating', '> 90 days old']], note: 'Recaptured leads are distributed evenly among PCs working in the same territory.' }
    ];
    ...
    const dailyProductivity = ['Read and respond to unreplied emails and Slack/Teams messages from the previous day.', 'Identify your top 20 warmest leads of the day.', 'Proceed with your call attempts, one lead after the other.', 'Allocate the last 1–2 hours of your day for sending emails and/or other admin tasks.'];
    ...
    const callFlows = [
      ...
      ['Publishing Consultation — Qualified Workflow', ['Introduction / build rapport', 'Must ask questions and confirm core-customer fit', 'Send welcome / opportunities email and estimate', 'Create estimate; update amount, closing date, stage, country, genre, publishing type, and currency', 'Convert lead to opportunity and account', 'Schedule closing / follow-up call, set stage 7, and close']],
      ['Publishing Consultation — Not Qualified Workflow', ['Introduction / build rapport', 'Must ask questions and assess the core-customer criteria', 'Use the appropriate outcome: foreign-language manuscript, cannot afford, printing only, already signed up, no manuscript / ghostwriting, 6- or 12-month lead, or technologically challenged', 'Update lead status to Low Value or Dead (reason code) and leave pertinent notes', 'Send the 6- or 12-month template when applicable']]
    ];
    function groupedCountries() { let groups = {}; dialCodes.forEach(([code, country, region]) => { let id = region + '|' + country; (groups[id] ??= { country, region, codes: [] }).codes.push('+' + code) }); return Object.values(groups).sort((a, b) => a.region.localeCompare(b.region) || a.country.localeCompare(b.country)) }
    function renderCountryGuide() { let table = document.querySelector('#countryGuideRows'); if (!table) return; let query = document.querySelector('#countryGuideSearch').value.trim().toLowerCase(), region = document.querySelector('#countryGuideRegion').value; let matches = groupedCountries().filter(x => (!region || x.region === region) && (`${x.country} ${x.codes.join(' ')} ${x.region}`).toLowerCase().includes(query)); table.innerHTML = matches.map(x => `<tr class="border-b border-slate-200 last:border-0"><td class="px-4 py-3 font-medium text-slate-800">${x.country}</td><td class="px-4 py-3 text-slate-600">${x.codes.join(', ')}</td><td class="px-4 py-3"><span class="rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-600">${x.region}</span></td><td class="px-4 py-3 text-emerald-700">Active</td></tr>`).join('') || '<tr><td colspan="4" class="px-4 py-8 text-center text-slate-500">No countries found.</td></tr>'; document.querySelector('#countryGuideCount').textContent = `${matches.length} countr${matches.length === 1 ? 'y' : 'ies'}` }
    function billingInvoicingPage() {
      ...
      const table = (heads, rows, widths = '') => `<div class="overflow-x-auto rounded-md border border-blue-100"><table class="w-full ${widths} text-left text-sm leading-5"><thead class="bg-[#edf5fd] text-[11px] font-bold uppercase text-[#17457e]"><tr>${heads.map(h => `<th class="px-3 py-2">${h}</th>`).join('')}</tr></thead><tbody>${rows.map(row => `<tr class="border-t border-blue-50">${row.map(cell => `<td class="px-3 py-2 align-top">${cell}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
      const discountRows = [['399 | US$1,745 | —','10%','CA$330 | AU$360 | US$262 | £187 | €225'],['899 | US$2,995 | —','10%','CA$57 | AU$85 | US$50 | £31 | €75'],['1,099 | US$3,499 | —','10%','CA$105 | AU$165 | US$82 | £61 | €135'],['1,799 | US$5,799 | —','10%','CA$110 | AU$170 | US$90 | £45 | €75'],['1,999 | US$7,999 | —','10%','CA$125 | AU$150 | US$200 | £85 | €1,035'],['2,999 | US$11,499 | —','10%','CA$950 | AU$2,000 | US$725 | £200 | €1,485']];
      ...
      return `<div class="w-full space-y-2.5 text-[#17457e]"><header class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-white via-[#f4f9ff] to-[#dceeff] px-6 py-8"><p class="text-xs font-bold uppercase tracking-[.18em] text-aus">Billing reference</p><h1 class="mt-1 text-3xl font-extrabold text-ink">Billing &amp; Invoicing</h1><p class="mt-2 max-w-3xl text-sm leading-6 text-[#5274a2]">Reference rules for discounts, payments, free services, and closing.</p></header>
      <div class="grid grid-cols-2 gap-2 rounded-lg border border-blue-100 bg-[#eff7ff] p-2 md:grid-cols-4">${[['percent','10% Cash Discount','(in selected invoice ranges)','bg-emerald-500'],['circle-dollar-sign','One Discount Type Only','Cash discount OR free services','bg-rose-500'],['calendar-days','Payment Plans','2–3 or 4–6 Months','bg-blue-600'],['move-horizontal','Non-card payments →','#payment_requests','bg-amber-500']].map(([ic,t,s,c])=>`<div class="flex items-center gap-2 border-r border-blue-100 px-2 last:border-0"><span class="grid h-7 w-7 shrink-0 place-items-center rounded-full ${c} text-white"><i data-lucide="${ic}" class="h-3.5 w-3.5"></i></span><div><p class="text-sm font-bold text-ink">${t}</p><p class="text-xs text-[#5274a2]">${s.startsWith('#')?`<a class="text-blue-700 underline" href="https://tellwell.slack.com/archives/${s.slice(1)}">${s}</a>`:s}</p></div></div>`).join('')}</div>
      <nav class="flex flex-wrap gap-1.5">${[['tag','Discounts','regular-discounting-guidelines'],['credit-card','Payments','payment-plans'],['gift','Free Services','free-services-guide'],['file-text','Closing','closing-guidelines']].map(([ic,t,id])=>`<a href="#bill-${id}" class="inline-flex items-center gap-2 rounded border border-blue-100 bg-white px-3 py-1.5 text-sm font-semibold text-[#17457e] hover:bg-blue-50"><i data-lucide="${ic}" class="h-3 w-3"></i>${t}</a>`).join('')}</nav>
      <div class="grid gap-2.5 lg:grid-cols-2"><div class="space-y-2.5">${panel('Regular Discounting Guidelines','tag',table(['Invoice','Cash Disc','Max Worth of Free Services'],discountRows))}${panel('Free Services Guide','gift',table(['Item (1)','Item (2)','Item (3)'],freeRows))}<section class="rounded-lg border border-rose-200 bg-rose-50 p-3"><h2 class="mb-1 flex items-center gap-2 text-sm font-extrabold text-rose-700"><i data-lucide="circle-alert" class="h-4 w-4"></i>Important Reminders</h2><ol class="list-decimal space-y-0.5 pl-5 text-xs leading-5 text-[#315b91]"><li>Discounts in the table are the ones that can be given independently at your discretion.</li><li>You can only give one type of discount: cash discount OR free services. You CANNOT give your client 10% cash discount and at the same time give a number of services for FREE.</li><li>These discounts should NOT be used in conjunction with another promo/special deal.</li></ol></section></div><div class="space-y-2.5">${panel('Payment Plans','calendar-days',table(['Plan','Details'],[['FULL PAYMENT',''],['2–3 MONTH PAYMENT PLAN','With Admin Fee. Payments must be 30 days apart.'],['4–6 MONTH PAYMENT PLAN','With Admin Fee. Payments must be 30 days apart.']]))}${panel('Payment Methods','credit-card',table(['Method','Details'],[['Credit/Debit Card','Process in Octavo'],['E-transfer','Send to admin@tellwell.ca'],['PayPal','https://www.paypal.me/Tellwell or send to admin@tellwell.ca'],['Wire Transfer/Bank Transfer/Direct Bank Deposit','Bank accounts are set up in each of the markets. For the complete bank details, see Tellwell Multicurrency Account Details for Bank Transfers.'],['Check','Have the check written out to Tellwell Talent, Inc. and have the check sent to: 2031 Store Street, Victoria, BC V8T 5L9.']]))}${panel('Company Forms','file-text','<ol class="list-decimal space-y-1 pl-5 text-xs"><li>Fill-up details.</li><li>Forward to JH for signing.</li><li><a href="#" class="text-blue-700 underline">Void Check Sample</a></li></ol>')}
      <div class="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs leading-4 text-[#17457e]"><h2 class="mb-1 flex items-center gap-2 text-sm font-bold"><i data-lucide="triangle-alert" class="h-4 w-4 text-amber-500"></i>For non-card payments, send a request to #payment_requests Slack channel so that payment can be processed. Template below:</h2><p class="rounded border border-amber-100 bg-white p-2">Hi @Mitch (JH), please confirm receipt of (PayPal/E-transfer/Wire Transfer/Bank Deposit) payment from (name of author) amount to expected amount. Please apply payment to project (insert Octavo project link). Thank you!</p></div></div></div>
      <section id="bill-closing-guidelines" class="card rounded-lg bg-white p-3"><h2 class="mb-2 flex items-center gap-2 text-lg font-extrabold text-ink"><i data-lucide="circle-dollar-sign" class="h-5 w-5 text-aus"></i>PR/PDF Drop Discounting and Closing Guidelines</h2><div class="grid gap-2 text-xs leading-4 md:grid-cols-2"><div class="rounded-md bg-[#f2f8ff] p-2"><p class="mb-1"><b>Note:</b> Please Log PDF Drop / Cover and/or PDF - Interior in the invoice</p><ol class="list-decimal space-y-0.5 pl-4"><li>Before closing the project, please make sure you have the file reviewed/verified by our design team.</li><li>Post request to design team to confirm file is print-ready; confirm trim size.</li><li>Provide quote for articles: <a class="text-blue-700 underline" href="#">I'm providing my own interior design—what does it cost?</a></li><li><a class="text-blue-700 underline" href="#">I'm providing my own cover design—what do I need to know?</a></li></ol></div><div class="rounded-md border border-blue-100 bg-[#f4faff] p-2"><b>ⓘ &nbsp;Other notes:</b><ol class="mt-1 list-[lower-alpha] space-y-1 pl-5"><li><b>Has editing:</b> The author must provide the Word document manuscript at start; then finalize the interior layout after edited files are fixed.</li><li><b>Has ebook:</b> Must provide properly packaged InDesign (.indd) files, otherwise the ebook will have to be fixed.</li></ol></div></div></section></div>`;
    }
    function countryCodePage() { return `<section class="mx-auto max-w-5xl"><div class="mb-6"><p class="text-sm font-semibold text-slate-500">Phone utility</p><h1 class="text-3xl font-bold text-slate-900">Country Code Guide</h1><p class="mt-2 text-slate-600">Look up LATAM and EUR country codes, or identify a phone number from its international code or local pattern.</p></div><section class="rounded-xl border border-slate-200 bg-white p-5"><h2 class="text-lg font-bold text-slate-900">Identify a phone number</h2><div class="mt-4 grid gap-3 md:grid-cols-[1fr_160px_auto]"><input id="countryPhone" type="tel" placeholder="e.g. +44 7700 900123" class="rounded-lg border border-slate-300 px-3 py-2.5 text-slate-900 outline-none focus:border-blue-600"><select id="countryRegion" class="rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-slate-900"><option value="">All regions</option><option value="LATAM">LATAM</option><option value="EUR">EUR</option></select><button id="countryDetect" class="rounded-lg bg-blue-700 px-5 py-2.5 font-semibold text-white hover:bg-blue-800">Identify</button></div><div id="countryResult" class="mt-4 hidden rounded-lg border px-4 py-3"><p id="countryResultStatus" class="font-semibold"></p><p id="countryResultDetail" class="mt-1 text-sm"></p></div><p class="mt-3 text-xs text-slate-500">Exact results require a number beginning with <b>+</b> or <b>00</b>. Local numbers are matched only against patterns for the selected region.</p></section><section class="mt-6 overflow-hidden rounded-xl border border-slate-200 bg-white"><div class="flex flex-col gap-3 border-b border-slate-200 p-5 sm:flex-row sm:items-center"><div><h2 class="text-lg font-bold text-slate-900">Country code directory</h2><p id="countryGuideCount" class="text-sm text-slate-500"></p></div><div class="sm:ml-auto flex flex-col gap-2 sm:flex-row"><input id="countryGuideSearch" placeholder="Search country or code" class="rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900 outline-none focus:border-blue-600"><select id="countryGuideRegion" class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900"><option value="">All regions</option><option value="LATAM">LATAM</option><option value="EUR">EUR</option></select></div></div><div class="overflow-x-auto"><table class="w-full min-w-[640px] text-left text-sm"><thead class="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th class="px-4 py-3">Country</th><th class="px-4 py-3">Country code</th><th class="px-4 py-3">Region</th><th class="px-4 py-3">Status</th></tr></thead><tbody id="countryGuideRows"></tbody></table></div></section></section>` }
    function companyDirectoryPage() { let teamRows = teamDirectory.map(([group, name, ext, email, location, market]) => `<tr class="border-b border-blue-50"><td class="px-3 py-3 text-xs font-bold text-aus">${group}</td><td class="px-3 py-3 font-semibold text-ink">${name}</td><td class="px-3 py-3">${ext}</td><td class="px-3 py-3 text-[#386198]">${email}</td><td class="px-3 py-3">${location}</td><td class="px-3 py-3">${market}</td></tr>`).join(''); return `<section><div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-white via-[#f4f9ff] to-[#dceeff] px-6 py-8"><p class="text-xs font-bold uppercase tracking-[.18em] text-aus">Internal directory</p><h1 class="mt-1 text-3xl font-extrabold text-ink">Directory & Company Info</h1><p class="mt-2 max-w-3xl text-sm leading-6 text-[#5274a2]">Teamwork makes the dream work. Find company details, the core and international teams, and the appropriate Slack channel.</p></div><div class="mt-5 grid gap-5 xl:grid-cols-[.9fr_1.4fr]"><section class="card rounded-xl bg-white p-5"><h2 class="text-lg font-extrabold text-ink">Basic company info</h2><dl class="mt-4 divide-y divide-blue-50">${companyInfo.map(([label, value]) => `<div class="grid gap-1 py-3 sm:grid-cols-[155px_1fr]"><dt class="text-sm font-bold text-ink">${label}</dt><dd class="text-sm leading-5 text-[#5274a2]">${value}</dd></div>`).join('')}</dl></section><section class="card overflow-hidden rounded-xl bg-white"><div class="border-b border-blue-100 p-5"><h2 class="text-lg font-extrabold text-ink">Team directory</h2><p class="mt-1 text-sm text-[#5274a2]">Core Group and International Team contact details.</p></div><div class="overflow-x-auto"><table class="w-full min-w-[720px] text-left text-sm"><thead class="bg-[#edf4fc] text-xs uppercase tracking-wide text-[#5274a2]"><tr><th class="px-3 py-3">Team</th><th class="px-3 py-3">Name</th><th class="px-3 py-3">Ext.</th><th class="px-3 py-3">Email address</th><th class="px-3 py-3">Location</th><th class="px-3 py-3">Market</th></tr></thead><tbody>${teamRows}</tbody></table></div></section></div><section class="card mt-5 rounded-xl bg-white p-5"><h2 class="text-lg font-extrabold text-ink">Slack channel guide</h2><div class="mt-4 grid gap-3 lg:grid-cols-2">${slackChannels.map(([channel, purpose]) => `<article class="rounded-lg border border-blue-50 bg-[#f8fbff] p-4"><h3 class="font-bold text-aus">${channel}</h3><p class="mt-1 text-sm leading-5 text-[#5274a2]">${purpose}</p></article>`).join('')}</div></section></section>` }
    function cadencesPage() {
      ...
      const priorities = callPriorities.map((item, index) => `<li class="grid grid-cols-[32px_1fr] items-center overflow-hidden border-b border-white/20 last:border-0"><span class="grid h-8 place-items-center text-xs font-extrabold text-black" style="background:${['#ff2222','#ff9d00','#fff500','#27ee17'][index]}">${index + 1}</span><span class="px-3 py-1.5 text-right text-xs font-bold text-[#8a5200]">${item}</span></li>`).join('');
      const workflows = callFlows.map(([title, steps]) => `<article class="card app-workflow rounded-xl bg-white p-5"><div class="mb-4 flex items-center gap-3 border-b border-blue-100 pb-3"><span class="grid h-8 w-8 place-items-center rounded-lg bg-blue-100 text-aus"><i data-lucide="git-branch" class="h-4 w-4"></i></span><h2 class="text-sm font-extrabold uppercase text-ink">${title}</h2></div><ol class="space-y-2">${steps.map((step, index) => `<li class="flex items-start gap-3"><span class="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-[#dcecff] text-xs font-extrabold text-aus">${index + 1}</span><span class="pt-0.5 text-sm leading-5 text-[#315b91]">${step}</span></li>`).join('')}</ol></article>`).join('');
      return `<section class="app-page"><div class="app-hero rounded-2xl px-6 py-7 sm:px-8"><div class="relative z-10 flex flex-col gap-5 lg:flex-row lg:items-end"><div class="flex-1"><p class="text-xs font-bold uppercase tracking-[.18em] text-aus">Daily operations</p><h1 class="mt-1 text-3xl font-extrabold text-ink">Cadences / Workflows</h1><p class="mt-2 max-w-3xl text-sm leading-6 text-[#5274a2]">Daily productivity, call priorities, and publishing-consultation workflows from the Cadences/Workflows spreadsheet.</p></div><div class="grid grid-cols-3 gap-2"><div class="app-metric rounded-xl px-3 py-2 text-center"><b class="block text-lg text-ink">4</b><span class="text-[10px] font-bold uppercase text-[#5274a2]">Priorities</span></div><div class="app-metric rounded-xl px-3 py-2 text-center"><b class="block text-lg text-ink">4</b><span class="text-[10px] font-bold uppercase text-[#5274a2]">Daily steps</span></div><div class="app-metric rounded-xl px-3 py-2 text-center"><b class="block text-lg text-ink">8</b><span class="text-[10px] font-bold uppercase text-[#5274a2]">Flows</span></div></div></div></div><div class="mt-5 grid gap-4 xl:grid-cols-[1fr_320px]"><section class="card rounded-xl bg-white p-5"><h2 class="app-section-title text-lg">Daily Productivity Guide</h2><div class="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">${productivity}</div></section><section class="card overflow-hidden rounded-xl bg-white"><h2 class="app-section-title border-b border-blue-100 px-4 py-3 text-sm">Order of Call Priority</h2><ol>${priorities}</ol></section></div><section class="mt-7"><h2 class="app-section-title mb-4 text-xl">Call Flows / Workflows</h2><div class="grid gap-4 xl:grid-cols-2">${workflows}</div></section></section>`;
    }
    function portalDetailPage(page) { let detail = portalDetail[page]; return `<section><div class="rounded-2xl bg-gradient-to-r from-white via-[#f4f9ff] to-[#dceeff] px-6 py-8"><p class="text-xs font-bold uppercase tracking-[.18em] text-aus">Tellwell PC Cheat Sheet</p><h1 class="mt-1 text-3xl font-extrabold text-ink">${detail.label}</h1><p class="mt-2 max-w-3xl text-sm leading-6 text-[#5274a2]">${detail.intro}</p></div><div class="mt-5 grid gap-4 lg:grid-cols-2">${detail.groups.map(([title, items]) => `<section class="card rounded-xl bg-white p-5"><h2 class="text-base font-extrabold text-ink">${title}</h2><div class="mt-4 space-y-3">${items.map(item => { let [heading, ...rest] = item.split('|'), body = rest.join('|'); return `<article class="rounded-lg border border-blue-50 bg-[#f8fbff] p-4"><h3 class="text-sm font-bold text-[#24518a]">${heading}</h3>${body ? `<p class="mt-1 text-sm leading-5 text-[#5274a2]">${body}</p>` : ''}</article>` }).join('')}</div></section>`).join('')}</div></section>` }
    function trainingPage() {
      const leadTags = leadTaggingGuide.map(([tag, detail]) => `<div class="grid gap-2 border-b border-blue-50 py-3 last:border-0 sm:grid-cols-[130px_1fr]"><dt class="text-sm font-extrabold text-ink">${tag}</dt><dd class="text-sm leading-5 text-[#315b91]">${detail}</dd></div>`).join('');
      const reassignment = reassignmentGuides.map(guide => `<article class="card rounded-xl bg-white p-5"><h2 class="text-base font-extrabold text-ink">Reassignment guide: ${guide.title}</h2><div class="mt-4 overflow-x-auto"><table class="w-full min-w-[390px] text-left text-sm"><thead class="bg-[#edf4fc] text-xs uppercase text-[#5274a2]"><tr><th class="px-3 py-2">Lead status</th><th class="px-3 py-2">And the age is</th></tr></thead><tbody>${guide.rows.map(([status, age]) => `<tr class="border-b border-blue-50 last:border-0"><td class="px-3 py-2.5 font-semibold text-ink">${status}</td><td class="px-3 py-2.5 text-[#315b91]">${age}</td></tr>`).join('')}</tbody></table></div>${guide.note ? `<p class="mt-3 text-xs leading-5 text-[#5274a2]">${guide.note}</p>` : ''}</article>`).join('');
      const stages = opportunityStages.map(([number, detail], index) => `<article class="flex gap-3 rounded-lg border border-blue-50 bg-[#f8fbff] p-3"><span class="grid h-10 w-10 shrink-0 place-items-center rounded-lg text-xl font-extrabold text-white" style="background:${['#1e5ed4','#155a69','#3e7e1f','#bd8600','#b75d00','#ab0505','#85301a'][index]}">${number}</span><p class="pt-0.5 text-sm leading-5 text-[#315b91]">${detail}</p></article>`).join('');
      const inactive = inactiveOpportunityGuides.map(([label, intro, reasons], index) => `<article class="card rounded-xl bg-white p-5"><h2 class="inline-block rounded-xl px-4 py-2 text-xl font-extrabold text-white" style="background:${index ? '#155a69' : '#3e7e1f'}">${label}</h2><p class="mt-4 text-sm leading-5 text-[#315b91]">${intro}</p><ul class="mt-3 list-disc space-y-1 pl-5 text-sm text-[#315b91]">${reasons.map(reason => `<li>${reason}</li>`).join('')}</ul></article>`).join('');
      return `<section><div class="rounded-2xl bg-gradient-to-r from-white via-[#f4f9ff] to-[#dceeff] px-6 py-8"><p class="text-xs font-bold uppercase tracking-[.18em] text-aus">Learning centre</p><h1 class="mt-1 text-3xl font-extrabold text-ink">Learning & Resources</h1><p class="mt-2 max-w-3xl text-sm leading-6 text-[#5274a2]">Training guides, tutorials, and policy documents from the Training/Memos/Tutorial Videos spreadsheet.</p></div><div class="mt-5 grid gap-4 xl:grid-cols-2"><section class="card rounded-xl bg-white p-5"><h2 class="text-base font-extrabold text-ink">A Quick Guide to Proper Lead Tagging</h2><dl class="mt-3">${leadTags}</dl></section><section class="card rounded-xl bg-white p-5"><h2 class="text-base font-extrabold text-ink">A Quick Guide to Proper Opportunity Stage Tagging</h2><div class="mt-4 space-y-2">${stages}</div></section></div><div class="mt-4 grid gap-4 xl:grid-cols-2">${reassignment}</div><section class="mt-4"><h2 class="mb-3 text-lg font-extrabold text-ink">A Quick Guide to Tagging Inactive/Lost Opportunities</h2><div class="grid gap-4 xl:grid-cols-2">${inactive}</div></section><section class="mt-6"><h2 class="mb-3 text-lg font-extrabold text-ink">Video &amp; Written Tutorials</h2><div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">${trainingVideos.map(([title, url]) => `<a href="${url}" target="_blank" rel="noopener noreferrer" class="card overflow-hidden rounded-xl bg-white transition hover:-translate-y-0.5 hover:shadow-soft"><div class="flex aspect-video items-center justify-center bg-gradient-to-br from-[#223343] to-[#16232e]"><span class="grid h-9 w-9 place-items-center rounded-full border-2 border-[#42657b] text-[#dce3e7]"><i data-lucide="play" class="ml-0.5 h-4 w-4 fill-current"></i></span></div><div class="p-4"><h3 class="text-sm font-bold leading-5 text-ink">${title}</h3><p class="mt-2 text-[11px] text-[#a36345]">Open tutorial <i data-lucide="external-link" class="ml-1 inline h-3 w-3"></i></p></div></a>`).join('')}</div></section><section class="card mt-6 rounded-xl bg-white p-5"><h2 class="text-base font-extrabold text-ink">Policy Resource Documents</h2><div class="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">${trainingPolicies.map(([title, url]) => `<a href="${url}" target="_blank" rel="noopener noreferrer" class="flex items-center justify-between rounded-lg border border-blue-100 bg-[#f8fbff] px-3 py-2.5 text-sm font-semibold text-[#24518a] hover:bg-blue-50"><span>${title}</span><i data-lucide="external-link" class="h-4 w-4 shrink-0 text-aus"></i></a>`).join('')}</div></section></section>`;
    }
    ...
    function tone(c) { return { blue: 'bg-blue-100 text-blue-600', teal: 'bg-teal-100 text-teal-600', violet: 'bg-violet-100 text-violet-600', orange: 'bg-orange-100 text-orange-600', emerald: 'bg-emerald-100 text-emerald-600', rose: 'bg-rose-100 text-rose-600', indigo: 'bg-indigo-100 text-indigo-600', cyan: 'bg-cyan-100 text-cyan-600' }[c] }
    function linkRow(x, c) { let k = key(x), s = saved.includes(k); return `<div class="resource-row group flex items-center gap-2 px-1 py-2"><i data-lucide="file-text" class="h-3.5 w-3.5 shrink-0 ${tone(c).split(' ')[1]}"></i><button class="open min-w-0 flex-1 truncate text-left text-[11px] font-medium text-[#24518a] hover:text-aus" data-key="${k}">${x.name}</button><button class="save rounded p-1 ${s ? 'text-amber-500' : 'text-[#9db4d1]'} hover:bg-blue-50" data-key="${k}" aria-label="Save resource"><i data-lucide="star" class="h-3.5 w-3.5" ${s ? 'fill=currentColor' : ''}></i></button><button class="copy rounded p-1 text-[#9db4d1] opacity-0 group-hover:opacity-100" data-url="${x.url}"><i data-lucide="copy" class="h-3.5 w-3.5"></i></button><a href="${x.url}" target="_blank" rel="noopener noreferrer" class="rounded-full border border-[#d8e7f8] px-2 py-0.5 text-[10px] font-semibold text-[#1971d0] hover:bg-blue-50">View</a></div>` }
    function renderMarketLinks() { let grid = document.querySelector('#marketLinkGrid'); if (!grid) return; document.querySelector('#marketTabs').innerHTML = Object.keys(websiteLinksByMarket).map(m => `<button class="marketTab rounded-full border px-3 py-1.5 text-xs font-bold ${m === activeMarket ? 'border-aus bg-aus text-white' : 'border-blue-100 bg-white text-[#386198] hover:bg-blue-50'}" data-market="${m}">${m}</button>`).join(''); grid.innerHTML = websiteLinksByMarket[activeMarket].map(([label, url]) => `<a href="${url}" target="_blank" rel="noopener noreferrer" class="flex items-center justify-between rounded-lg border border-[#d8e7f8] bg-white px-3 py-2.5 text-sm font-semibold text-[#24518a] hover:bg-blue-50"><span>${label}</span><i data-lucide="external-link" class="h-4 w-4 text-aus"></i></a>`).join('') }
    function render() { if (activePage !== 0) return; let market = document.querySelector('#marketLinks'); if (!market) { document.querySelector('#categoryGrid').insertAdjacentHTML('beforebegin', '<section id="marketLinks" class="card mb-4 rounded-xl bg-white p-4"><div class="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center"><div><h2 class="text-sm font-extrabold text-ink">Website Links by Market</h2><p class="mt-1 text-xs text-[#5274a2]">Official Tellwell pages from the Consultation Resources sheet.</p></div><div id="marketTabs" class="flex flex-wrap gap-2 sm:ml-auto"></div></div><div id="marketLinkGrid" class="grid gap-2 sm:grid-cols-2 xl:grid-cols-3"></div></section>'); } renderMarketLinks(); let q = (document.querySelector('#resourceSearch').value + ' ' + document.querySelector('#globalSearch').value).trim().toLowerCase(), count = 0; document.querySelector('#categoryGrid').innerHTML = data.map(c => { let list = allItems().filter(x => x.cat === c.id).filter(x => (!q || `${x.name} ${x.title} ${x.type} ${x.url}`.toLowerCase().includes(q)) && (filter === 'All' || x.type === filter) && (!savedMode || saved.includes(key(x)))); count += list.length; if (!list.length) return ''; let show = list.slice(0, 4), isForm = c.id === 'quote'; return `<article class="card flex min-h-[265px] flex-col rounded-xl bg-white p-4"><div class="mb-2 flex gap-3"><div class="icon-ball grid h-11 w-11 shrink-0 place-items-center rounded-full ${tone(c.color)}"><i data-lucide="${icons[c.id]}" class="h-5 w-5"></i></div><div><h3 class="text-[13px] font-extrabold leading-[1.2] text-[#0b3470]">${c.title}</h3><p class="mt-1 text-[11px] leading-[1.35] text-[#5274a2]">${c.desc}</p></div></div><div class="${isForm ? 'mt-[-4px]' : 'mt-auto'}">${show.map(x => linkRow(x, c.color)).join('')}${isForm ? '' : `<button class="listCategory mt-3 inline-flex items-center gap-2 text-[11px] font-bold text-[#0871df]" data-category="${c.id}">View all ${list.length} resources <i data-lucide="arrow-right" class="h-3.5 w-3.5"></i></button>`}</div></article>` }).join('') || '<div class="col-span-full rounded-xl bg-white p-12 text-center text-sm text-[#5274a2]">No resources match your search.</div>'; document.querySelector('#resultInfo').classList.toggle('hidden', !q && !savedMode && filter === 'All'); document.querySelector('#resultInfo').textContent = `${count} resource${count === 1 ? '' : 's'} found`; document.querySelector('#filters').innerHTML = ['All', 'Guides', 'Templates', 'Forms', 'Reference', 'Samples'].map(f => `<button data-filter="${f}" class="filter rounded-full border px-4 py-2 text-xs font-semibold ${filter === f ? 'border-aus bg-aus text-white' : 'border-[#d8e7f8] bg-white text-[#5274a2] hover:bg-blue-50'}">${f}</button>`).join(''); popular(); recents(); lucide.createIcons() }
    function popular() { let p = ['Publishing Guide', 'Production Process Overview', 'Interior Layout Samples', 'Print Prices', 'Tellwell Size Matrix']; document.querySelector('#popular').innerHTML = p.map(n => { let x = allItems().find(a => a.name === n); return `<button class="open rounded-lg border border-blue-50 p-2 text-left hover:bg-blue-50" data-key="${key(x)}"><i data-lucide="${icons[x.cat]}" class="mb-2 h-4 w-4 text-aus"></i><span class="block truncate text-[10px] font-semibold text-[#24518a]">${n}</span><span class="mt-1 block text-[10px] text-aus">View →</span></button>` }).join('') }
    function recents() { let a = recent.length ? recent : allItems().slice(0, 4).map((x, i) => ({ ...x, when: ['Viewed 2 hours ago', 'Viewed 4 hours ago', 'Opened yesterday', 'Viewed 2 days ago'][i] })); document.querySelector('#recent').innerHTML = a.slice(0, 4).map(x => `<button class="open flex w-full items-center gap-2 border-b border-blue-50 py-2 text-left last:border-0" data-key="${key(x)}"><i data-lucide="file-text" class="h-4 w-4 text-aus"></i><span class="min-w-0 flex-1 truncate text-[11px] font-medium text-[#24518a]">${x.name}</span><span class="hidden text-[10px] text-[#7893b6] sm:block">${x.when}</span><i data-lucide="chevron-right" class="h-4 w-4 text-aus"></i></button>`).join('') }
    ...
    function showResource(x) { recent = [{ ...x, when: 'Opened just now' }, ...recent.filter(y => key(y) !== key(x))].slice(0, 8); localStorage.setItem('aus-recent', JSON.stringify(recent)); document.querySelector('#resourceModalLabel').textContent = x.title; document.querySelector('#resourceModalTitle').textContent = x.name; document.querySelector('#resourceModalBody').innerHTML = `<div class="rounded-xl bg-[#f5faff] p-4"><p class="text-sm leading-6 text-[#5274a2]">Open this resource in a new tab, copy its URL, or save it to your bookmarks for quick access.</p><p class="mt-3 break-all rounded-lg bg-white p-3 text-xs text-[#5274a2]">${x.url}</p></div><div class="mt-5 flex flex-wrap justify-end gap-2"><button class="modalCopy rounded-lg border border-blue-100 px-4 py-2 text-sm font-bold text-ink" data-url="${x.url}"><i data-lucide="copy" class="mr-1 inline h-4 w-4"></i> Copy URL</button><button class="launchURL rounded-lg bg-aus px-4 py-2 text-sm font-bold text-white" data-url="${x.url}">Open resource <i data-lucide="external-link" class="ml-1 inline h-4 w-4"></i></button></div>`; document.querySelector('#resourceModal').classList.remove('hidden'); document.querySelector('#resourceModal').classList.add('flex'); document.body.classList.add('modal-open'); lucide.createIcons(); render() }
    function showCategory(id) { let c = data.find(x => x.id === id), list = allItems().filter(x => x.cat === id); document.querySelector('#resourceModalLabel').textContent = 'All resources'; document.querySelector('#resourceModalTitle').textContent = c.title; document.querySelector('#resourceModalBody').innerHTML = `<p class="mb-4 text-sm text-[#5274a2]">${c.desc}</p><div class="space-y-2">${list.map(x => `<button class="open flex w-full items-center gap-3 rounded-lg border border-blue-100 p-3 text-left hover:bg-[#f5faff]" data-key="${key(x)}"><i data-lucide="file-text" class="h-4 w-4 text-aus"></i><span class="flex-1 text-sm font-semibold text-ink">${x.name}</span><i data-lucide="chevron-right" class="h-4 w-4 text-aus"></i></button>`).join('')}</div>`; document.querySelector('#resourceModal').classList.remove('hidden'); document.querySelector('#resourceModal').classList.add('flex'); document.body.classList.add('modal-open'); lucide.createIcons() }
    function open(k) { let x = find(k); if (x) showResource(x) }
    function navMarkup() { document.querySelector('#sideNav').innerHTML = nav.map((n, i) => `<button data-page="${i}" class="nav-link flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm ${i === activePage ? 'active font-bold' : 'text-blue-100'}"><i data-lucide="${n[1]}" class="h-5 w-5"></i><span>${n[0]}</span></button>`).join('') }
    function sectionPageMarkup(page) { let section = tabPages[page], [title, icon] = nav[page]; return `<section class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-white via-[#f4f9ff] to-[#dceeff] px-6 py-8 sm:px-8"><div class="relative z-10 flex max-w-3xl items-start gap-4"><div class="icon-ball grid h-14 w-14 shrink-0 place-items-center rounded-full bg-[#d9edff] text-[#1169cc]"><i data-lucide="${icon}" class="h-7 w-7"></i></div><div><p class="text-xs font-bold uppercase tracking-[.18em] text-aus">${section.eyebrow}</p><h1 class="mt-1 text-2xl font-extrabold tracking-tight text-ink sm:text-[32px]">${title}</h1><p class="mt-2 text-sm leading-6 text-[#5274a2] sm:text-base">${section.intro}</p></div></div><i data-lucide="${icon}" class="absolute right-7 top-5 hidden h-32 w-32 text-[#5da5eb] opacity-30 md:block"></i></section><section class="mt-5 grid gap-4 lg:grid-cols-2">${section.groups.map(group => `<article class="card rounded-xl bg-white p-5"><div class="mb-4 flex items-center gap-3"><div class="icon-ball grid h-10 w-10 place-items-center rounded-full bg-blue-100 text-aus"><i data-lucide="${group.icon}" class="h-5 w-5"></i></div><h2 class="text-base font-extrabold text-ink">${group.title}</h2></div><ul class="space-y-2">${group.items.map(item => `<li class="flex gap-3 rounded-lg bg-[#f7fbff] px-3 py-2.5 text-sm leading-5 text-[#315b91]"><i data-lucide="check-circle-2" class="mt-0.5 h-4 w-4 shrink-0 text-aus"></i><span>${item}</span></li>`).join('')}</ul></article>`).join('')}</section>` }
    function navigate(page) { activePage = page; navMarkup(); document.querySelector('#pageContent').innerHTML = page === 0 ? consultationView : page === 1 ? countryCodePage() : page === 3 ? billingInvoicingPage() : page === 4 ? companyDirectoryPage() : page === 5 ? cadencesPage() : page === 6 ? trainingPage() : portalDetail[page] ? portalDetailPage(page) : sectionPageMarkup(page); if (page === 0) render(); else { renderNavigationLinks(); lucide.createIcons(); if (page === 1) renderCountryGuide() } document.querySelector('#globalSearch').value = ''; document.querySelector('#sidebar').classList.add('hidden'); document.querySelector('#sidebar').classList.remove('flex'); document.querySelector('#overlay').classList.add('hidden') }
